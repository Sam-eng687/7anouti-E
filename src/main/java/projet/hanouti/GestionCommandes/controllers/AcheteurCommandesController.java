...
public class AcheteurCommandesController implements Initializable {
    ...
    /** Guard to avoid marking changes while the detail UI is being populated */
    ...
    private void onAdresseChanged() {
        if (isLoadingDetail || selectedCommande == null) return;
        ...
        new Thread(() -> {
            ...
            javafx.application.Platform.runLater(() -> {
                adresseValide = valid;
                if (valid) {
                    boolean changed = !normalized.equals(originalAdresse);
                    adresseValidIcon.setText("✅");
                    ...
                    if (changed) {
                        ...
                        adresseHint.setText("✅ Adresse identique à l'originale");
                        pendingAdresseValue = null;
                        pendingAdresse = false;
                        updateValidationBox();
                    }
    ...
    public class MapBridge {
        public void onMapClick(String address, double lat, double lng) {
            javafx.application.Platform.runLater(() -> {
                ...
                adresseValide = true;
                if (!address.equals(originalAdresse)) {
                    adresseValidIcon.setText("📍");
...
// Delete the old refused commande from DB
    ...
    private class LigneCell extends ListCell<LigneCommande> {
        ...
        LigneCell(boolean editable) {
            ...
            btnDelete.getStyleClass().add("btn-annuler");
            btnDelete.setStyle("-fx-padding:4 9; -fx-font-size:11px;");
            btnDelete.setFocusTraversable(false);
            row.getChildren().addAll(lNom, sp, spinner, btnDelete, lPrix);
            ...
            spinner.valueProperty().addListener((obs, oldV, newV) -> {
                ...
                if (suppressSpinnerEvent || isLoadingDetail || ligne == null || newV == null || newV.equals(oldV)) return;
            ...
            btnDelete.setOnAction(e -> {
                if (!editable || selectedCommande == null) return;
                LigneCommande ligne = getItem();
                if (ligne == null) return;

                ligneService.removeLigne(ligne.getIdLigne());
                listLignes.getItems().remove(ligne);
                detNbProduits.setText(listLignes.getItems().size() + " produit(s)");
                ...
                pendingQuantite = true;
                markDirty("Produit supprimé — cliquez 'Valider' pour confirmer");
            });
        }
