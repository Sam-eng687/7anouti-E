...
import projet.hanouti.wejden.gui.components.ShopAssistantChat;
import projet.hanouti.wejden.entities.CampagneMarketing;
import projet.hanouti.wejden.entities.ConseilsMarketing;
import projet.hanouti.wejden.entities.StatistiquesVentes;
import projet.hanouti.wejden.services.CampagneMarketingService;
import projet.hanouti.wejden.services.ConseilsMarketingService;
import projet.hanouti.wejden.services.ClaudeAIService;
...
//import wejden.gui.components.ShopAssistantChat;
import projet.hanouti.wejden.services.GeminiMarketingService;
import projet.hanouti.wejden.utils.*;
import projet.hanouti.wejden.utils.QrCodeFx;
import projet.hanouti.wejden.gui.components.SidebarNav;
import projet.hanouti.wejden.services.*;
import projet.hanouti.wejden.utils.ConseilShareServer;
import projet.hanouti.wejden.utils.GoogleCalendarPromoLink;
...
public class HanoutiDashboard extends Application {
    ...
    private void applyTheme(boolean dark) {
        ...
        PDFExportManager.setDarkMode(dark);
    ...
    private final PrayerTimeService prayerService = new PrayerTimeService();
    ...
    private SidebarNav sidebarNavRef;
    ...
    /** Racine principale (standalone : VBox ; embed : VBox barre recherche + contenu) */
    private javafx.scene.layout.Pane rootRef;
    private StackPane globalOverlay;
    private boolean shellEmbedMode;
    private Label embedThemeIconLbl;
    ...
    public void start(Stage stage) {
        ...
        // Démarrer le générateur automatique de conseils IA
        System.out.println("[HanoutiDashboard] Démarrage du générateur automatique de conseils IA...");
        ...
        StackPane.setAlignment(chatFab, javafx.geometry.Pos.BOTTOM_RIGHT);
    ...
    /**
     * Marketing intégré au shell vendeur : barre de recherche seule sous l'en-tête app (IA / PDF dans dashboard.fxml).
     */
    public StackPane createShellEmbedRoot(Stage ownerStage) {
        this.shellEmbedMode = true;
        this.primaryStage = ownerStage;
        applyTheme(SessionManager.getInstance().isDarkMode());

        // Démarrer le générateur automatique de conseils IA (si pas déjà démarré)
        if (!conseilsIaService.isAutoGenerationRunning()) {
            System.out.println("[HanoutiDashboard] Démarrage du générateur automatique de conseils IA (mode embed)...");
            conseilsIaService.startAutoGeneration();
        }
        ...
        // Libellé module (stats / conseils / campagnes) — mis à jour par naviguerVers ; non affiché en embed.
        ...
        HBox searchRow = buildSearchIaThemeCluster(false, true);

        HBox embedSearchBar = new HBox(12);
        embedSearchBar.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
        embedSearchBar.setPrefHeight(52);
        embedSearchBar.setMinHeight(52);
        embedSearchBar.setMaxHeight(52);
        embedSearchBar.setPadding(new Insets(0, 12, 0, 14));
        String topBarBg = darkMode ? BG_SIDEBAR : "#ffffff";
        String topBarBorder = darkMode ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.1)";
        embedSearchBar.setStyle(
                "-fx-background-color: " + topBarBg + ";"
                        + "-fx-border-color: " + topBarBorder + ";"
                        + "-fx-border-width: 0 0 1 0;"
                        + (darkMode ? "" : "-fx-effect: dropshadow(gaussian,rgba(0,0,0,0.04),8,0,0,2);"));
        this.topBarRef = embedSearchBar;
        Region embedSearchSpacer = new Region();
        HBox.setHgrow(embedSearchSpacer, Priority.ALWAYS);
        embedSearchBar.getChildren().addAll(embedSearchSpacer, searchRow);

        VBox column = new VBox(embedSearchBar, contentArea, liveBarRef);
        column.setStyle("-fx-background-color: " + BG_DEEP + ";");
        VBox.setVgrow(contentArea, Priority.ALWAYS);

        this.rootRef = column;

        globalOverlay = new StackPane(column);
        Button chatFab = new Button("\uD83E\uDD16");
        chatFab.setTooltip(new Tooltip("Assistant Hanouti (Gemini) — boutique & app"));
        chatFab.setStyle(
                "-fx-font-size: 20px; -fx-background-radius: 99; -fx-min-width: 54; -fx-min-height: 54; -fx-max-width: 54; -fx-max-height: 54;"
    ...
    public void navigateEmbedTo(String module) {
        if (!shellEmbedMode) {
            return;
        }
        naviguerVers(module);
    ...
    public void applyShellTheme(boolean dark) {
        if (!shellEmbedMode) {
            return;
        }
        applyTheme(dark);
        if (embedThemeIconLbl != null) {
            embedThemeIconLbl.setText(darkMode ? "\uD83C\uDF19" : "\u2600");
        }
        updateThemeStyles();
    }
    ...
    private Label buildIaActiveBadgePill() {
    ...
    private Button buildThemeToggleButton() {
        ...
        this.embedThemeIconLbl = themeIcon;
        themeIcon.setStyle("-fx-font-size:20px;");
        Button themeBtn = new Button();
        themeBtn.setGraphic(themeIcon);
        themeBtn.setPrefSize(44, 44);
        themeBtn.setMinSize(44, 44);
        themeBtn.setMaxWidth(48);
        themeBtn.setStyle(
                "-fx-background-color:" + (darkMode ? "rgba(255,255,255,0.1)" : "rgba(59,130,246,0.1)") + ";"
                        + "-fx-background-radius:10;"
                        + "-fx-border-color:" + (darkMode ? "rgba(255,255,255,0.2)" : "rgba(59,130,246,0.3)") + ";"
        ...
        themeBtn.setOnAction(e -> {
            ...
            SessionManager.getInstance().setDarkMode(darkMode);
            updateThemeStyles();
        });
    ...
    /**
     * Recherche globale + badge IA + bouton thème (app standalone) ;
     * si {@code searchOnly}, uniquement la zone de recherche (shell vendeur intégré).
     */
    private HBox buildSearchIaThemeCluster(boolean withThemeToggle, boolean searchOnly) {
        ...
        if (searchOnly) {
            globalSearchField.setMaxWidth(280);
            HBox.setHgrow(globalSearchField, Priority.NEVER);
        } else {
        ...
        final java.util.List<CampagneMarketing> campsCache;
        final java.util.List<StatistiquesVentes> statsCache;
        final java.util.List<ConseilsMarketing> conseilsCache;
        java.util.List<CampagneMarketing> _c = null;
        java.util.List<StatistiquesVentes> _s = null;
        java.util.List<ConseilsMarketing> _k = null;
        try {
        ...
        globalSearchField.textProperty().addListener((obs, old, query) -> {
            ...
            java.util.List<CampagneMarketing> campStarts = new java.util.ArrayList<>();
            java.util.List<CampagneMarketing> campContains = new java.util.ArrayList<>();
            for (CampagneMarketing c : campsCache) {
            ...
            java.util.List<CampagneMarketing> campsSorted = new java.util.ArrayList<>(campStarts);
            campsSorted.addAll(campContains);
            int foundC = 0;
            for (CampagneMarketing c : campsSorted) {
                if (foundC >= 5)
                    break;
            ...
            for (StatistiquesVentes s : statsCache) {
            ...
            for (String ref : prodsSorted) {
                ...
                double totalRev = statsCache.stream()
                        ...
                        .mapToDouble(StatistiquesVentes::getRevenuTotal)
                        .sum();
                resultsBox.getChildren().add(buildSearchResultRow(
                        "\uD83D\uDCE6", refClean,
                        String.format("Produit  \u00B7  %.0f TND  \u00B7  %s  \u00B7  %s",
                                totalRev,
                                ExchangeRateService.format(totalRev, "EUR"),
                                ExchangeRateService.format(totalRev, "USD")),
                        NEON_GREEN,
                        () -> {
                            naviguerVers("stats");
                            globalSearchField.clear();
            ...
            java.util.List<ConseilsMarketing> conseilStarts = new java.util.ArrayList<>();
            java.util.List<ConseilsMarketing> conseilContains = new java.util.ArrayList<>();
            for (ConseilsMarketing c : conseilsCache) {
            ...
            java.util.List<ConseilsMarketing> conseilsSorted = new java.util.ArrayList<>(
                    conseilStarts);
            conseilsSorted.addAll(conseilContains);
            int foundK = 0;
            for (ConseilsMarketing c : conseilsSorted) {
                if (foundK >= 2)
                    break;
        ...
        if (searchOnly) {
            HBox wrapSearch = new HBox(12, searchBox);
            wrapSearch.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
            searchBox.setMaxWidth(Region.USE_PREF_SIZE);
            this.themeBtnRef = null;
            this.embedThemeIconLbl = null;
            return wrapSearch;
        }

        Label iaBadge = buildIaActiveBadgePill();

        if (withThemeToggle) {
            Button themeBtn = buildThemeToggleButton();
            HBox wrap = new HBox(12, searchBox, iaBadge, themeBtn);
            wrap.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
            HBox.setHgrow(searchBox, Priority.ALWAYS);
            return wrap;
        }
        this.themeBtnRef = null;
        this.embedThemeIconLbl = null;
        HBox wrapNoTheme = new HBox(12, searchBox, iaBadge);
        wrapNoTheme.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
        HBox.setHgrow(searchBox, Priority.ALWAYS);
        return wrapNoTheme;
    }
    ...
    private HBox buildTopBar() {
        ...
        HBox searchIaTheme = buildSearchIaThemeCluster(true, false);
        ...
        bar.getChildren().addAll(menuBtn, logoFlow, sep, topBarModuleLabel, spacer, searchIaTheme);
    ...
    private HBox buildLiveAlertBar() {
        ...
        liveBadge.setAlignment(javafx.geometry.Pos.CENTER);
        ...
        try {
            ...
            for (Map<String, Object> p : produits) {
                ...
                if (stock < 10) {
                    ...
                    alert.setAlignment(javafx.geometry.Pos.CENTER);
    ...
    private VBox buildSidebar() {
        ...
        logoZone.setAlignment(javafx.geometry.Pos.CENTER);
        ...
        SidebarNav sidebarNav = new SidebarNav();
        ...
        dbZone.setAlignment(javafx.geometry.Pos.CENTER);
        ...
        btnPdf.setOnAction(e -> PDFExportManager.exportRapportComplet(primaryStage));
    ...
    private void naviguerVers(String module) {
        ...
        javafx.application.Platform.runLater(() -> {
            ...
            // Barre LIVE : uniquement sur le module Ventes & performances (stats)
            if (liveBarRef != null) {
                boolean onStats = "stats".equals(currentModule);
                liveBarRef.setVisible(onStats);
                liveBarRef.setManaged(onStats);
            }
    ...
    private ScrollPane buildModuleStats() {
        ...
        view.setPadding(shellEmbedMode ? new Insets(10, 12, 14, 12) : new Insets(28));
        ...
        String revenuEurUsd = ExchangeRateService.format(totalRevenu, "EUR")
                + "  \u2022  "
                + ExchangeRateService.format(totalRevenu, "USD");
        ...
        iaStrip.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
        ...
        pillRow.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
    ...
    private VBox buildAlertsSection() {
        ...
        try {
            ...
            java.sql.Statement st = MyBD.getInstance().getConnection().createStatement();
        ...
        try {
            String sql = "SELECT COUNT(*) as nb, MIN(date_genere) as plus_ancien FROM conseils_ia WHERE etat = 'EN_ATTENTE'";
            java.sql.Statement st = MyBD.getInstance().getConnection().createStatement();
            java.sql.ResultSet rs = st.executeQuery(sql);
            if (rs.next()) {
                int nb = rs.getInt("nb");
    ...
    private VBox buildWeatherWidget() {
        ...
        new Thread(() -> {
            ...
            Platform.runLater(() -> {
                ...
                Label icon = new Label(WeatherService.getWeatherEmoji(code));
                ...
                Label desc = new Label(WeatherService.getWeatherDescription(code)
            ...
            for (int i = 0; i < series.getData().size(); i++) {
                ...
                if (d.getNode() != null) {
                    ...
                    String tipText = d.getXValue() + "\n"
                            ...
                            + ExchangeRateService.format(val, "EUR") + "  "
                            + ExchangeRateService.format(val, "USD");
    ...
    private void updateHexFromMapList(StackPane holder, List<java.util.Map<String, Object>> list) {
        ...
        hexBox.setAlignment(javafx.geometry.Pos.CENTER);
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            VBox txt = new VBox(2);
            txt.setAlignment(javafx.geometry.Pos.CENTER);
    ...
    private int getVuesProduit(String produitId) {
        try {
            ...
            String sql = "SELECT COALESCE(SUM(nb_interaction), 0) FROM Interaction_Utilisateur WHERE id_produit = ? AND type_interaction = 'VIEW'";
    ...
    private void updateHexGridFull(StackPane holder, String p1, String p2, String rev1, String rev2) {
        ...
        leg.setAlignment(javafx.geometry.Pos.CENTER);
    ...
    private HBox buildDetailedRow(String name, String period, String sem, String sales, String rev, String ret,
        ...
        bBox.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
    ...
    private HBox buildStatRow(String name, String sub, String vStatus, String rStatus, String cStatus, String sStatus,
        ...
        rBox.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
        ...
        cBox.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
        ...
        sBox.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
    ...
    private ScrollPane buildModuleConseils() {
        ...
        view.setPadding(shellEmbedMode ? new Insets(10, 12, 14, 12) : new Insets(28, 32, 32, 32));
        view.setStyle("-fx-background-color: transparent;");
        ...
        HBox autoGenBar = new HBox(12);
        autoGenBar.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
        autoGenBar.setPadding(new Insets(12, 16, 12, 16));
        String autoGenBg = darkMode ? "rgba(16,185,129,0.08)" : "#d1fae5";
        String autoGenBorder = darkMode ? "rgba(16,185,129,0.2)" : "#6ee7b7";
        autoGenBar.setStyle(
                "-fx-background-color:" + autoGenBg + ";" +
                "-fx-background-radius:12;" +
                "-fx-border-color:" + autoGenBorder + ";" +
                "-fx-border-width:1;" +
                "-fx-border-radius:12;");

        // Status indicator
        javafx.scene.shape.Circle statusDot = new javafx.scene.shape.Circle(5);
        statusDot.setFill(Color.web(NEON_GREEN));
        FadeTransition pulse = new FadeTransition(Duration.seconds(1.5), statusDot);
        pulse.setFromValue(1.0);
        ...
        VBox statusTexts = new VBox(2);
        Label statusTitle = new Label("Génération Automatique Active");
        statusTitle.setStyle("-fx-font-size:13px;-fx-font-weight:bold;-fx-text-fill:" + NEON_GREEN + ";");
        Label statusDesc = new Label("Analyse des interactions toutes les " + 
                                     conseilsIaService.getAutoGenerationInterval() + " minutes");
        statusDesc.setStyle("-fx-font-size:11px;-fx-text-fill:" + TEXT_3 + ";");
        statusTexts.getChildren().addAll(statusTitle, statusDesc);

        Region autoGenSpacer = new Region();
        HBox.setHgrow(autoGenSpacer, Priority.ALWAYS);

        // Manual trigger button
        Button generateNowBtn = new Button("⚡ Générer Maintenant");
        String genBtnBg = darkMode ? "rgba(16,185,129,0.15)" : "#a7f3d0";
        String genBtnText = darkMode ? NEON_GREEN : "#047857";
        String genBtnBorder = darkMode ? NEON_GREEN : "#10b981";
        generateNowBtn.setStyle(
                "-fx-background-color:" + genBtnBg + ";" +
                "-fx-text-fill:" + genBtnText + ";" +
                "-fx-font-size:12px;-fx-font-weight:bold;" +
                "-fx-cursor:hand;-fx-padding:8 16;" +
                "-fx-background-radius:8;" +
                "-fx-border-color:" + genBtnBorder + ";" +
                "-fx-border-width:1.5;-fx-border-radius:8;");
        generateNowBtn.setOnAction(e -> {
            generateNowBtn.setDisable(true);
            generateNowBtn.setText("⏳ Génération...");
            new Thread(() -> {
                conseilsIaService.generateConseilsNow();
                Platform.runLater(() -> {
                    generateNowBtn.setDisable(false);
                    generateNowBtn.setText("⚡ Générer Maintenant");
                    showNotification("Conseils IA", "Nouveaux conseils générés avec succès!", "success");
                    // Refresh the page
                    naviguerVers("conseils");
                });
            }).start();
        ...
        autoGenBar.getChildren().addAll(statusDot, statusTexts, autoGenSpacer, generateNowBtn);
        view.getChildren().add(autoGenBar);
        ...
        int nouveaux = conseilsIaService.compter("EN_ATTENTE");
        ...
        filtresTabs.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
    ...
    private void refreshConseilsCards(javafx.scene.layout.FlowPane container, List<Map<String, Object>> allData,
        ...
        List<Map<String, Object>> filtered = allData.stream().filter(c -> {
            ...
            if (filter.equals("Actifs"))
                return "EN_ATTENTE".equals(c.get("etat"));
    ...
    private ScrollPane buildModuleCampagnes() {
        ...
        // KPI
        HBox kpiRow = new HBox(14);
        kpiRow.setPadding(shellEmbedMode ? new Insets(12, 12, 0, 12) : new Insets(20, 32, 0, 32));
        ...
        String budgetSub = String.format("%.0f TND  \u2022  %s  \u2022  %s",
                totalBudget,
                ExchangeRateService.format(totalBudget, "EUR"),
                ExchangeRateService.format(totalBudget, "USD"));
        String depenseSub = String.format("%.0f TND  \u2022  %s  \u2022  %s",
                totalDepense,
                ExchangeRateService.format(totalDepense, "EUR"),
                ExchangeRateService.format(totalDepense, "USD"));
        kpiRow.getChildren().addAll(
        ...
        iaSection.setPadding(shellEmbedMode ? new Insets(12, 12, 0, 12) : new Insets(20, 32, 0, 32));
        ...
        campSection.setPadding(shellEmbedMode ? new Insets(12, 12, 14, 12) : new Insets(20, 32, 32, 32));
    ...
    private void openAjouterCampagneDialog() {
        ...
        btnSave.setOnAction(e -> {
            ...
            try {
                java.sql.PreparedStatement chk = MyBD.getInstance().getConnection()
            ...
            try {
                java.sql.PreparedStatement chk2 = MyBD.getInstance().getConnection()
                        .prepareStatement(
    ...
    private VBox buildBarChart(List<StatistiquesVentes> stats, String filter) {
        ...
        bars.setAlignment(javafx.geometry.Pos.BOTTOM_LEFT);
        ...
        for (int i = 0; i < count; i++) {
            ...
            group.setAlignment(javafx.geometry.Pos.BOTTOM_CENTER);
            HBox pair = new HBox(4);
            pair.setAlignment(javafx.geometry.Pos.BOTTOM_CENTER);
    ...
    private VBox buildPieChart(String filter) {
        ...
        hexContainer.setAlignment(javafx.geometry.Pos.CENTER);
    ...
    private VBox buildCampCard(CampagneMarketing c, String accentColor, int pct) {
        ...
        financialRow.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
        ...
        Label budDevises = new Label(
                ExchangeRateService.format(c.getDepense(), "EUR")
                        + "  \u2022  "
                        + ExchangeRateService.format(c.getDepense(), "USD"));
        ...
        Label revDevises = new Label(hasRevenu
                ? ExchangeRateService.format(revenuAttribue, "EUR")
                        + "  \u2022  "
                        + ExchangeRateService.format(revenuAttribue, "USD")
        ...
        budBarRow.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
        ...
        StackPane.setAlignment(budFill, javafx.geometry.Pos.CENTER_LEFT);
        ...
        revBarRow.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
        ...
        StackPane.setAlignment(revFill, javafx.geometry.Pos.CENTER_LEFT);
        ...
        Label freshLbl = new Label(ExchangeRateService.isFresh() ? "\u26A1" : "\u26A1");
        ...
        javafx.scene.control.Tooltip freshTip = new javafx.scene.control.Tooltip(
                ExchangeRateService.isFresh()
                        ? "Taux mis a jour a " + ExchangeRateService.getLastUpdateTime()
    ...
    private ScrollPane styledScroll(VBox content) {
        return styledScroll((javafx.scene.Node) content);
    }
    ...
    private void exportToPDF(List<Map<String, Object>> data) {
        ...
        if (file != null) {
            try {
                ReportingService.generatePDF(data, file);
    ...
    private void showEmailDialog(List<Map<String, Object>> data) {
        ...
        dialog.showAndWait().ifPresent(email -> {
            ReportingService.sendEmail(email, data);
    ...
    private final VendeurService vendeurService = new VendeurService();
    ...
    private void openEmailTargetDialog() {
        ...
        btnEnvoyer.setOnAction(e -> {
            ...
            new Thread(() -> {
                try {
                    List<Map<String, Object>> produits = statsService.getDataAsMaps();
                    ReportingService.sendVendeurEmail(email, nom, produits, joursFinaux);
    ...
    private void openDestockageConfirmDialog(int idConseil, int idProduit, String prodName, VBox card) {
        ...
        ok.setOnAction(e -> {
            // La méthode accepter(idConseil) gère déjà le passage du stock à 0 
            // si le conseil est de type "Destockage".
    ...
    private void openBundleConfigDialog(int idConseil, String produitPrincipal) {
        ...
        bundleBtns.setAlignment(javafx.geometry.Pos.CENTER_RIGHT);
