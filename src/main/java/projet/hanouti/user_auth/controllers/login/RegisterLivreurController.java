...
public class RegisterLivreurController {
    ...
    @FXML private TextField permisField;
    @FXML private TextField zoneLivraisonField;
    ...
    @FXML private Button chooseZoneBtn;

    private double zoneLat = 0;
    private double zoneLng = 0;
    ...
    public void initialize() {
        ...
        chooseZoneBtn.setOnAction(e -> openZonePopup());
        zoneLivraisonField.setOnMouseClicked(e -> openZonePopup());
    ...
    private void onRegister() {
        ...
        delay.setOnFinished(ev -> {
            try {
                ...
                String vehicule = getText(vehiculeField);
                String permis = getText(permisField);
                String zone = getText(zoneLivraisonField);

                if (vehicule.isBlank() || permis.isBlank() || zone.isBlank()) {
                    showError("Veuillez remplir les informations de livraison.");
                    shakeNode(formCard);
                    registerBtn.setText(originalText);
                    registerBtn.setDisable(false);
                    registerBtn.setOpacity(1.0);
                ...
                User newUser = new User(nom, prenom, dateStr, email, numTel, pass,
                        imagePath, Role.livreur, Status.Unbanned);
                ...
                newUser.setAdresse(zoneLivraisonField.getText());
                newUser.setVehicule(vehicule);
                newUser.setPermis(permis);
                newUser.setZoneLivraison(zone);
        ...
        if (faceEnabled) {
            try {
                ...
                for (int i = 0; i < 3; i++) {
                    String path = CameraCapture.capturePhoto();
                }
    ...
    private void openZonePopup() {
        Dialog<String> dialog = new Dialog<>();
        dialog.setTitle("Choisir zone de livraison");
        dialog.setHeaderText("Tapez votre ville ou zone");
        ...
        Label help = new Label("Écrivez au moins 3 lettres puis cliquez Rechercher.");
        help.setStyle("-fx-text-fill: #6B7280; -fx-font-size: 12px;");

        Button searchBtn = new Button("Rechercher");
        searchBtn.setStyle(
                "-fx-background-color: linear-gradient(to right, #4338CA, #6366F1);" +
                        "-fx-text-fill: white; -fx-font-weight: 800; -fx-background-radius: 10;" +
                        "-fx-padding: 8 16; -fx-cursor: hand;"
        );

        VBox box = new VBox(12, searchField, searchBtn, listView, help);
        box.setPrefWidth(430);
        dialog.getDialogPane().setContent(box);

        java.util.Map<String, double[]> coordinates = new java.util.HashMap<>();

        searchBtn.setOnAction(e -> {
            String q = searchField.getText() == null ? "" : searchField.getText().trim();

            if (q.length() < 3) {
                help.setText("Tapez au moins 3 lettres.");
                return;
            }
            ...
            try {
                ...
                while (m.find()) {
                    ...
                    coordinates.put(name, new double[]{lat, lon});
                }

                if (listView.getItems().isEmpty()) {
                    help.setText("Aucune zone trouvée.");
                } else {
                    help.setText("Sélectionnez une zone puis Confirmer.");
                }

            } catch (Exception ex) {
                help.setText("Erreur de recherche zone.");
                ex.printStackTrace();
            }
        });
        ...
        dialog.showAndWait().ifPresent(selected -> {
            zoneLivraisonField.setText(selected);

            double[] pos = coordinates.get(selected);
            if (pos != null) {
                zoneLat = pos[0];
                zoneLng = pos[1];
            }
