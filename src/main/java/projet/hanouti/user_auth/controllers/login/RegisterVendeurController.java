...
import projet.hanouti.common.utils.PhoneCountryDetector;
import projet.hanouti.common.utils.WelcomeEmailTemplate;
...
public class RegisterVendeurController {
    ...
    @FXML private TextField adresseEntrepriseField;
    @FXML private TextField typeProduitField;

    @FXML private Button chooseEntrepriseMapBtn;

    private double entrepriseLat = 0;
    private double entrepriseLng = 0;
    ...
    public void initialize() {
        ...
        projet.hanouti.common.utils.FormValidator.setupRegisterValidation(
                nameField,           errNom,
        ...
        chooseEntrepriseMapBtn.setOnAction(e -> openEntrepriseMapPopup());
        adresseEntrepriseField.setOnMouseClicked(e -> openEntrepriseMapPopup());
    ...
    private void onRegister() {
        ...
        boolean valid = projet.hanouti.common.utils.FormValidator.validateAll(
                nameField,           errNom,
                prenomField,         errPrenom,
                dateNaissField,      errDate,
                emailField,          errEmail,
        ...
        delay.setOnFinished(ev -> {
            try {
                ...
                if (entreprise.isBlank() || adresse.isBlank() || typeProduit.isBlank()) {
                    showError("Veuillez remplir les informations de l'entreprise.");
                    shakeNode(formCard);
                    registerBtn.setText(originalText);
                    registerBtn.setDisable(false);
                    registerBtn.setOpacity(1.0);
                ...
                if (entrepriseLat == 0 || entrepriseLng == 0) {
                    showError("Veuillez choisir l'adresse de l'entreprise depuis la map.");
                    shakeNode(formCard);
                    registerBtn.setText(originalText);
                    registerBtn.setDisable(false);
                    registerBtn.setOpacity(1.0);
                ...
                User newUser = new User(nom, prenom, dateStr, email, numTel, pass,
                        imagePath, Role.vendeur, Status.Unbanned);
                ...
                newUser.setTypeProduit(typeProduit);
                newUser.setAdresse(adresseEntrepriseField.getText());
    ...
    private void updateCountryBadge(String phone) {
        ...
        PhoneCountryDetector.PhoneInfo info =
                projet.hanouti.common.utils.PhoneCountryDetector.detect(phone);
    ...
    private void openEntrepriseMapPopup() {
        ...
        searchField.setPromptText("Ex: Tunis, Sfax, Ben Guerdane...");

        Button searchBtn = new Button("Rechercher");
        Button confirmBtn = new Button("Confirmer");

        Label infoLabel = new Label("Recherchez une adresse ou cliquez sur la map.");
        infoLabel.setStyle("-fx-text-fill: #6B7280; -fx-font-size: 12px;");

        WebView webView = new WebView();
        webView.setPrefSize(760, 460);
        ...
        engine.setOnAlert(event -> {
            ...
            if (data != null && data.startsWith("LOC|")) {
                try {
                    ...
                    String json = projet.hanouti.common.utils.GeoService.reverseGeocode(lat, lng);
                    ...
                    if (m.find()) {
                        selectedAddress[0] = m.group(1)
                                .replace("\\/", "/")
                                .replace("\\u0026", "&");
                    } else {
                        selectedAddress[0] = "Lat: " + lat + ", Lng: " + lng;
                    }

                    infoLabel.setText(selectedAddress[0]);

                } catch (Exception ex) {
                    selectedAddress[0] = "Lat: " + selectedLat[0] + ", Lng: " + selectedLng[0];
                    infoLabel.setText(selectedAddress[0]);
                    ex.printStackTrace();
                }
            }
        ...
        searchBtn.setOnAction(e -> {
            ...
            if (q.length() < 3) {
                infoLabel.setText("Tapez au moins 3 lettres.");
                return;
            }

            try {
                String json = projet.hanouti.common.utils.GeoService.searchZone(q);
                ...
                if (m.find()) {
                    ...
                    infoLabel.setText("Adresse introuvable.");
                }

            } catch (Exception ex) {
                infoLabel.setText("Erreur geocoding.");
                ex.printStackTrace();
            }
        });
        ...
        confirmBtn.setOnAction(e -> {
            if (selectedLat[0] == 0 && selectedLng[0] == 0) {
                infoLabel.setText("Choisissez une position avant confirmer.");
                return;
            }

            adresseEntrepriseField.setText(selectedAddress[0]);
            entrepriseLat = selectedLat[0];
            entrepriseLng = selectedLng[0];
        ...
        HBox bottom = new HBox(10, infoLabel, confirmBtn);
        bottom.setStyle("-fx-padding: 12; -fx-alignment: center-right;");

        VBox root = new VBox(8, top, webView, bottom);
    ...
    <body>
        ...
        <script>
            ...
            map.on('click', function(e) {
                ...
                alert("LOC|" + e.latlng.lat + "|" + e.latlng.lng);
            });
        </script>
    </body>
    </html>
