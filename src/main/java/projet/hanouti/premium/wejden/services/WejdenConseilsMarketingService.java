...
public class WejdenConseilsMarketingService {
    ...
    public List<ConseilsMarketing> getData() {
        ...
        String req = "SELECT c.id_conseil, c.id_produit, c.type, c.description, c.score, c.etat, c.urgence, c.date_genere, p.nom " +
                     ...
                     "UNION ALL " +
                     "SELECT conseil_id as id_conseil, CAST(c.produit_id AS CHAR) as id_produit, type_conseil as type, description, 0 as score, IF(applique, 'ACCEPTE', 'EN_ATTENTE') as etat, 'NORMAL' as urgence, date_generation as date_genere, p.nom " +
        ...
        try {
            ...
            while (rs.next()) {
                ...
                c.setImpactEstime(rs.getString("urgence"));
    ...
    public void updateStatut(int id, String etat) {
        ...
        try {
            String normalizedEtat = normalizeEtat(etat);
            PreparedStatement pst = MyBD.getInstance().getConnection().prepareStatement(req);
            pst.setString(1, normalizedEtat);
            pst.setInt(2, id);
            pst.executeUpdate();
            
            // Also update old table if exists
            String oldReq = "UPDATE conseils_marketing SET applique = ? WHERE conseil_id = ?";
            PreparedStatement pstOld = MyBD.getInstance().getConnection().prepareStatement(oldReq);
            pstOld.setBoolean(1, "ACCEPTE".equals(normalizedEtat));
            pstOld.setInt(2, id);
    ...
    private String normalizeEtat(String etat) {
        if ("ACCEPTE".equalsIgnoreCase(etat) || "APPLIQUE".equalsIgnoreCase(etat)) return "ACCEPTE";
        if ("IGNORE".equalsIgnoreCase(etat) || "IGNOREE".equalsIgnoreCase(etat)) return "IGNORE";
        return "EN_ATTENTE";
    }
