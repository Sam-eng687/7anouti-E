package projet.hanouti.premium.wejden.services;
...
public class WejdenCampagneMarketingService {
    ...
    public void addEntity(CampagneMarketing c) {
        int vid = currentVendorId();
        String checkSql = "SELECT COUNT(*) FROM campagne_marketing WHERE nom = ? AND vendor_id = ?";
        try {
        ...
        String req = "INSERT INTO campagne_marketing (vendor_id, nom, type_action, objectif, statut, date_debut, date_fin, budget_alloue, budget_depense, score_ia) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try {
            PreparedStatement pst = MyBD.getInstance().getConnection().prepareStatement(req);
            pst.setInt(1, vid);
            pst.setString(2, c.getNomCampagne());
            pst.setString(3, c.getCanal());
            pst.setString(4, c.getObjectif());
            pst.setString(5, c.getStatut());
            pst.setDate(6, c.getDateDebut());
            pst.setDate(7, c.getDateFin());
            pst.setDouble(8, c.getBudget());
            pst.setDouble(9, c.getDepense());
            pst.setDouble(10, c.getIaScore());
            pst.executeUpdate();
    ...
    public List<CampagneMarketing> getData() {
        ...
        int vid = currentVendorId();
        for (String req : java.util.List.of(
                "SELECT * FROM campagne_marketing WHERE vendor_id=" + vid,
                "SELECT * FROM campagne_marketing WHERE id_vendeur=" + vid,
                "SELECT * FROM campagne_marketing")) {
            List<CampagneMarketing> list = fetchCampagnes(req);
            if (!list.isEmpty()) {
                return list;
            }
        }
    ...
    private List<CampagneMarketing> fetchCampagnes(String req) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                for (int i = 1; i <= meta.getColumnCount(); i++) {
                    ...
                    if (colName.equals("id") || colName.equals("campagne_id")) {
                        idCol = i;
                        break;
                    }
                }
