...
import edu.hanouti.services.*;
import edu.hanouti.entities.*;
...
public class PDFExportManager {
    ...
    private static String buildJsonRapportComplet() throws Exception {
        ...
        int nouveau = (int) conseilsList.stream().filter(m -> "NOUVEAU".equals(m.get("etat"))).count();
    ...
    private static void runPython(String mode, String jsonData, String outPath) throws Exception {
        ...
        // Trouver pdf_export.py
        File projectDir = new File(System.getProperty("user.dir"));
        File script = new File(projectDir, "pdf_export.py");
        if (!script.exists()) {
            script = new File(projectDir.getParentFile(), "pdf_export.py");
            projectDir = script.getParentFile();
        }
    ...
    private static String formatPdfExportError(Throwable e) {
        ...
        if (raw.contains("reportlab") || raw.contains("ModuleNotFoundError")) {
            return "Le module Python « reportlab » est requis pour générer le PDF.\n\n"
                    + "Dans un terminal, à la racine du projet (là où se trouve requirements.txt) :\n"
