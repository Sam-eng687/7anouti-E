...
public class StatistiquesVentesService implements IService<StatistiquesVentes> {
    ...
    public void addEntity(StatistiquesVentes stat) {
        ...
        try {
            PreparedStatement pst = MyConnection.getConnection().prepareStatement(req);
    ...
    public double getRevenuMoisCourant() {
        ...
        try { Statement st = MyConnection.getConnection().createStatement(); ResultSet rs = st.executeQuery(sql); if(rs.next()) return rs.getDouble(1); } catch(SQLException ignored){}
    ...
    public int getVenduMoisCourant() {
        ...
        try { Statement st = MyConnection.getConnection().createStatement(); ResultSet rs = st.executeQuery(sql); if(rs.next()) return rs.getInt(1); } catch(SQLException ignored){}
