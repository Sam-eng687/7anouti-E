...
def _revenu_produit(p):
    """Revenu TND pour une ligne produit (JSON Java envoie souvent 'revenu', parfois 'revenu_total')."""
    v = p.get("revenu")
    if v is None or v == "":
        v = p.get("revenu_total")
    try:
        return float(v or 0)
...
def page_stats(c, d, page=1, total=1):
    ...
    if chart_top > CONTENT_BOT + 140:  # Plus d'espace pour la légende
        ...
        max_r = max((_revenu_produit(p) for p in produits), default=0.0)
        if max_r <= 0:
            max_r = 1.0
        BH_MAX = 60
        BW     = 36
